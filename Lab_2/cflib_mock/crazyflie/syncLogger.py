...
class SyncLogger:
    ...
    def __next__(self):
        if not self._running:
            raise StopIteration
        # Build a data dict based on current scenario values
        data = {}
        for name, fmt in self.logconf.variables:
            # Map variable names to scenario keys
            if name == 'pm.vbat':
                ...
                data[name] = None
        # Sleep a short time to allow real time to advance for the student's end_time wait
        time.sleep(0.05)
